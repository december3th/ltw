...
let check = () => {
    ...
    if (!checkName(userName)) {
        ...
        error1.innerHTML = "không được bỏ trống";
    } else {
        error1.innerHTML = "";
    }
    if (!checkEmail(email)) {
        let error2 = document.getElementById("error2");
        error2.innerHTML = "nhập đúng cấu trúc gmail";
    } else {
        error2.innerHTML = "";
        if (!checkEmailExist(email)) {

            error2.innerHTML = "gmail đã tồn tại";
        } else {
            error2.innerHTML = "";
        }
    }
    if (!checkPass(password)) {
        ...
        error3.innerHTML = "mật khẩu tối thiểu 8 kí tự";
    } else {
        error3.innerHTML = "";
    }
    if (!checkAddress(address)) {
        let error4 = document.getElementById("error4");
        error4.innerHTML = "không được bỏ trống";
    } else {
        error4.innerHTML = "";
    }
    if (!checkRepeatpass(repeatpass, password)) {
        let error5 = document.getElementById("error5");
        error5.innerHTML = "mật khẩu được nhắc lại không đúng";
    } else {
        error5.innerHTML = "";
    }
}
