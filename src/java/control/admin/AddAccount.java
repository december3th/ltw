...
public class AddAccount extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String user_password_raw = request.getParameter("user_password");
        String address = request.getParameter("address");
        DAO dao = new DAO();
        User check = dao.checkUserExist(user_email);
        final String secretKey = "TrinhVietHieu0112!!!@@@";
        String user_password = AES.encrypt(user_password_raw, secretKey);
        if(check == null) {
            dao.signUp(user_name, user_email, user_password, address);
